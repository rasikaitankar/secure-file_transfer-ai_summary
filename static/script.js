...
uploadForm.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      document.getElementById("resStorage").textContent  = data.storage === "google_drive" ? "Google Drive" : "Local";
...
function renderFileList(files) {
  ...
  files.forEach((f) => {
    const storageIcon = f.storage === "google_drive" ? " Drive" : "Local";
...
async function setDelay(ms) {
  ...
  try {
    ...
    el.innerHTML = ms > 0
      ? `<div style="color:var(--accent3);font-size:0.85rem;">${data.message}</div>`
